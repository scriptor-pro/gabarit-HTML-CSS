(async () => {
  ...
  const contentSuggestions = document.getElementById(
    "content-font-suggestions",
  );
  ...
  const colorSurfaceAltValue = document.getElementById(
  ...
  const colorLineStrongValue = document.getElementById(
    "color-line-strong-value",
  );
  ...
  const embeddedCardTemplate = document.getElementById(
    "embedded-card-template",
  );
  const insertTemplateCardButton = document.getElementById(
    "insert-template-card",
  );
  const clearTemplateCardsButton = document.getElementById(
    "clear-template-cards",
  );
  ...
  const storageKeys = {
    ...
    fontUi: "font-ui-config",
    fontContent: "font-content-config",
  ...
  const isValidFontConfig = (value) =>
    isPlainObject(value) &&
    typeof value.family === "string" &&
    value.family.length > 0 &&
    typeof value.weight === "string" &&
    /^[0-9]{3}$/.test(value.weight);

  const toStoredColorConfig = (value) => {
    ...
    for (const key of keys) {
      const normalized = normalizeHex(value[key]);
      if (normalized) {
        output[key] = normalized.toLowerCase();
      }
    }
  ...
  const shiftHexColor = (hex, ratio) => {
    ...
    const adjust = (channel) =>
      ratio >= 0 ? channel + (255 - channel) * ratio : channel * (1 + ratio);

    const toHex = (channel) =>
      clampColor(adjust(channel)).toString(16).padStart(2, "0");
  ...
  const inferClassFromSlug = (slug) =>
    /(serif|slab|garamond|baskerville|bodoni|mincho|didone|roman|antiqua)/i.test(
      slug,
    )
      ? "serif"
      : "sans";
  ...
  const loadBunnyCatalog = async () => {
    const xmlText = await fetchTextWithRetry(
      "https://fonts.bunny.net/sitemap.xml",
      {
        cache: "force-cache",
        timeoutMs: 6500,
        retries: 1,
    ...
    if (xml.querySelector("parsererror")) {
      throw new Error("Sitemap Bunny invalide");
    }
    ...
    if (groups.length === 0) {
      ...
      Array.from(select.options).forEach((option) => {
        const haystack = normalizeText(
          [option.textContent || "", option.value || ""].join(" "),
        );
  ...
  const resetAllTokens = () => {
    storage.remove(storageKeys.preset);
    storage.remove(storageKeys.colorConfig);
    storage.remove(storageKeys.fontUi);
    storage.remove(storageKeys.fontContent);
  ...
  const setIframeDemoLoaded = (isLoaded) => {
    ...
    if (toggleIframeDemoButton) {
      toggleIframeDemoButton.setAttribute(
        "aria-expanded",
        isLoaded ? "true" : "false",
      );
    }
  ...
  const enableTrackDemo = () => {
    ...
    const syncTrackMode = () => {
      if (
        !videoWithTrack.textTracks ||
        videoWithTrack.textTracks.length === 0
      ) {
    ...
    videoWithTrack.addEventListener("error", () => {
      report(
        "Sous-titres: video indisponible (verifie la connexion et CORS). ",
      );
    });
  ...
  const updateContrastNotice = (inkHex, bgHex) => {
    ...
    if (ratio !== null && ratio < 4.5) {
      ...
      contrastWarning.textContent =
        "Contraste texte/fond faible (" +
        ratio.toFixed(2) +
        ":1). Vise au moins 4.5:1.";
    } else {
  ...
  const applyColorConfig = (config) => {
    ...
    const surfaceAlt =
      config.surfaceAlt ||
      shiftHexColor(config.surface, mode === "dark" ? 0.08 : -0.03);
    const inkSoft =
      config.inkSoft ||
      shiftHexColor(config.ink, mode === "dark" ? -0.28 : 0.35);
    const lineStrong =
      config.lineStrong ||
      shiftHexColor(config.line, mode === "dark" ? 0.25 : -0.15);
    ...
    storage.setJSON(storageKeys.colorConfig, {
  ...
  const applyAestheticPreset = (presetKey) => {
    const key = aestheticPresets[presetKey]
      ? presetKey
      : defaultAestheticPreset;
    if (aestheticPreset) {
      aestheticPreset.value = key;
    }
    applyColorConfig(aestheticPresets[key]);
    storage.set(storageKeys.preset, key);
  };
  ...
  const collectColorConfig = () => ({
    ...
    surfaceAlt: colorSurfaceAlt
      ? colorSurfaceAlt.value
      : defaultColorConfig.surfaceAlt,
    ...
    lineStrong: colorLineStrong
      ? colorLineStrong.value
      : defaultColorConfig.lineStrong,
  });
  ...
  const applyTheme = (mode) => {
    ...
    themeToggle.textContent =
      resolved === "dark" ? "Passer en mode clair" : "Passer en mode fonce";
    storage.set(storageKeys.themeMode, resolved);
  };
  ...
  const applyFontConfig = (group) => {
    ...
    const familyName =
      selected.dataset.name || selected.textContent.trim() || "Inter";
    const familyClass = selected.dataset.class === "serif" ? "serif" : "sans";
    const normalizedWeight = String(
      Math.min(900, Math.max(100, Number(weightInput.value) || 400)),
    );
    const fallbackStack =
      familyClass === "serif"
    ...
    if (activeTarget) {
      activeTarget.textContent =
        "Police active: " + familyName + " (" + normalizedWeight + ")";
    }

    storage.setJSON(isUi ? storageKeys.fontUi : storageKeys.fontContent, {
      family: slug,
      weight: normalizedWeight,
    });
  };

  const uiSavedRaw = storage.getJSON(storageKeys.fontUi);
  const contentSavedRaw = storage.getJSON(storageKeys.fontContent);
  const uiSaved = isValidFontConfig(uiSavedRaw) ? uiSavedRaw : null;
  const contentSaved = isValidFontConfig(contentSavedRaw)
    ? contentSavedRaw
    : null;

  const hydrateBunnyCatalogAsync = async () => {
    try {
      ...
      if (bunnySlugs.length > 0) {
        ...
        if (catalogStatus) {
          catalogStatus.textContent =
            bunnySlugs.length + " polices disponibles";
        }
      } else if (catalogStatus) {
        catalogStatus.textContent =
          "Catalogue Bunny vide, liste locale conservee.";
      }
    } catch (error) {
      console.error(error);
      if (catalogStatus) {
        catalogStatus.textContent =
          "Catalogue Bunny indisponible, liste locale conservee.";
      }
    }
  };

  const scheduleBunnyCatalogHydration = () => {
    const run = () => {
      void hydrateBunnyCatalogAsync();
    };
  ...
  const savedTheme = storage.get(storageKeys.themeMode);
  const systemPrefersDark =
    window.matchMedia &&
    window.matchMedia("(prefers-color-scheme: dark)").matches;
  applyTheme(savedTheme || (systemPrefersDark ? "dark" : "light"));

  const storedPreset = storage.get(storageKeys.preset);
  const storedColorConfig = toStoredColorConfig(
    storage.getJSON(storageKeys.colorConfig),
  );
  if (storedPreset && aestheticPresets[storedPreset]) {
    ...
    applyColorConfig({
      ...
      surfaceAlt:
        storedColorConfig?.surfaceAlt || defaultColorConfig.surfaceAlt,
      ...
      lineStrong:
        storedColorConfig?.lineStrong || defaultColorConfig.lineStrong,
    });
  }

  scheduleBunnyCatalogHydration();

  const initFontModule = () => {
    ...
    contentSearch.addEventListener("input", (event) => {
      const query = event.target.value;
      const previous = contentFamily.value;
      ...
      if (contentFamily.value !== previous) {
        applyFontConfig("content");
      }
    });
  ...
  const initInteractiveModule = () => {
    if (
      openDialogButton &&
      demoDialog &&
      typeof demoDialog.showModal === "function"
    ) {
    ...
    if (toggleInertButton && inertZone) {
      const updateInertState = () => {
        ...
        if (inertState) {
          inertState.textContent =
            "Etat: " + (isInert ? "inert actif" : "actif");
        }
      };
    ...
    if (copyClipboardButton && clipboardText) {
      copyClipboardButton.addEventListener("click", async () => {
        try {
          await copyTextToClipboard(clipboardText.value || "");
          if (clipboardState) {
    ...
    if (dragSource && dropTarget) {
      ...
      const dragPayload = "DIY CSS draggable chip";
      const setKeyboardDrag = (active) => {
        keyboardDragActive = active;
        dragSource.setAttribute("aria-grabbed", active ? "true" : "false");
        dropTarget.classList.toggle("is-active", active);
        if (dropState && active) {
          dropState.textContent =
            "Etat: source prete. Depose dans la zone cible.";
        }
      };

      const commitDrop = (payload) => {
      ...
      dragSource.addEventListener("keydown", (event) => {
        if (event.key !== "Enter" && event.key !== " ") {
          return;
        }
        event.preventDefault();
        setKeyboardDrag(!keyboardDragActive);
      });
      ...
      dropTarget.addEventListener("keydown", (event) => {
        ...
        event.preventDefault();
        commitDrop(dragPayload + " (clavier)");
      });
    }
  };

  const initThemeAndPaletteModule = () => {
  ...
  const initExportModule = () => {
    exportThemeButton.addEventListener("click", async () => {
      try {
        let baseCss = "";
        try {
          baseCss = await fetchTextWithRetry("style.css", {
            cache: "no-store",
            timeoutMs: 4000,
            retries: 0,
          });
        ...
        console.error(error);
        alert(
          "Export impossible. Ouvre la page via un serveur local (ex: Live Server). ",
        );
      }
    });
  };

  const initEmbeddedModule = () => {
    if (toggleIframeDemoButton && iframeDemo) {
      setIframeDemoLoaded(true);
      toggleIframeDemoButton.addEventListener("click", () => {
        const expanded =
          toggleIframeDemoButton.getAttribute("aria-expanded") === "true";
        setIframeDemoLoaded(!expanded);
      });
    }

    if (
      insertTemplateCardButton &&
      ...
      insertTemplateCardButton.addEventListener("click", () => {
        ...
        if (createdAt) {
          createdAt.textContent =
            "Creee le " + new Date().toLocaleString("fr-FR");
        }
